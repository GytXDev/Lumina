...
class CarsRepository {
  ...
  Future<void> reportOrder(OrderCars order, BuildContext context) async {
    print("Début de reportOrder...");
    final ordersRef = FirebaseFirestore.instance.collection('orders');
    final alertsRef = FirebaseFirestore.instance.collection('alerts');

    try {
      print("Mise à jour de l'ordre avec isAlert à true...");
      await ordersRef.doc(order.orderId).update({
        'isAlert': true,
      });

      print("Ajout d'une nouvelle alerte dans la collection 'alerts'...");
      await alertsRef.add({
        'orderId': order.orderId,
        'userId': order.requesterId,
        'userName': order.requesterName,
      ...
      // Récupérer un admin
      print("Récupération des informations de l'administrateur...");
      UserModel? adminUser = await getOneAdmin();
      if (adminUser != null) {
        print("Admin trouvé : ${adminUser.username}");
        ...
        String alertMessage = AppLocalizations.of(context)
            .translateWithVariables('alertMessage', {
          'carName': order.carName,
          'requesterName': order.requesterName,
        });

        print("Envoi du message d'alerte...");
        // ignore: use_build_context_synchronously
        chatRepository.sendTextMessage(
          // ignore: use_build_context_synchronously
          context: context,
          textMessage: alertMessage,
          receiverId: order.userId, // ID du propriétaire du véhicule
          senderData: adminUser, // Informations de l'administrateur
        );
