        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              decoration: InputDecoration(
                labelText:
                    '${AppLocalizations.of(context).translate('priceLabel')} (${user != null ? determineCurrency(user!.latitude, user!.longitude).toString().split('.').last : ''})',
                fillColor: Coolors
                    .greyDark, // Assurez-vous que `Coolors.greyDark` est défini correctement.
                suffixText:
                    ' ${user != null ? determineCurrency(user!.latitude, user!.longitude).toString().split('.').last : ''}',
              ),
